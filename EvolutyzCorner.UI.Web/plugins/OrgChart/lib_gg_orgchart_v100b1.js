...
( function (window, undefined) {
    ...
    window.ggOrgChart = {
        ...
        render: function (options, json_file)
        {
             ...
             options_heap = [];
             data_heap = [];
             oc_first_json_load_initiated = false;
    ...
    var defaultOptions = {
        ...
        box_border_radius: 0,//8,                 // border radius of boxes in pixels
        box_border_width: 1,//2,                  // border with of boxes in pixels
        ...
        images_base_url: '/Images/Profile-Images/', //'./images/',         // base url of the images to be embeeded in boxes, with a trailing slash
        images_size:[60,60], //[160, 160],              // size (x, y) of the images to be embeeded inside boxes
    ...
    function oc_text_limit (options, node) 
    {
        ...
        //if (typeof node.children == "undefined") chary commented
        if (typeof node.children == typeof undefined) {
            return;
        }
    ...
    {
        options.box_color='#fff';
        options.box_border_color = '#039be5';
        if (node === null)
            return;
        ...
        // now draw the box
        //
        // chary code for box border color 
        options.box_color = (node.box_color != null) ? options.box_color = node.box_color : options.box_color;
        options.box_border_color = (node.box_border_color != null) ? options.box_border_color = node.box_border_color : options.box_border_color;
        ...
        if (typeof options.box_html_template == "string") {
            ...
            // chary code for color filling box
            options.box_color = (node.box_color != null) ? options.box_color = node.box_color : options.box_color;
    ...
    function oc_print (options) {
        ...
        //var divContents = document.getElementById("familytree_chart").innerHTML;
        var divContents = $('#familytree_chart').html()
        var printWindow = window.open('', '', 'height=900,width=4092');
