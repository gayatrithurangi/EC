...

{
    ...
    public class MyHub : Hub
    {
        ...
        public void Connect(string userName, string password)
        {
            ...
            var ctx = new EvolutyzCornerDataEntities();



            var getinfo =
                 (from m in ctx.Users
                  where m.Usr_LoginId == userName && m.Usr_Password == password
                  select new {  m.Usr_UserID, m.Usr_Username, m.Usr_ActiveStatus }).FirstOrDefault();

            //var getinfo = (from m in ctx.Users
            //               where m.Usr_LoginId == userName && m.Usr_Password == password
            //               select m.Usr_Username).FirstOrDefault();

            //var info = (from m in ctx.Users where select m).ToList();
            ...
            try
            {
                ...
                //if ((int)userInfo.Usr_ActiveStatus == 0)
                //{
                ...
                //}
                    ...
                    UsersList.Add(new UserInfo { ConnectionId = id, AdminID = getinfo.Usr_UserID, UserName = userName ,UserGroup = getinfo.Usr_ActiveStatus.ToString(), freeflag = "1", tpflag = "1" });
                    //whether it is Admin or User now both of them has userGroup and I Join this user or admin to specific group 
                    Groups.Add(Context.ConnectionId, getinfo.Usr_ActiveStatus.ToString());
                    Clients.Caller.onConnected(id, userName, getinfo.Usr_UserID, getinfo.Usr_ActiveStatus.ToString());
